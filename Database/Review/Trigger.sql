...
--tang ma the loai
go
create trigger trig_tangMaTL on LoaiVe
instead of insert
as
begin
	...
	insert into NhanVien values(@maloaive, @tenloaive, @giave)
end
--tang ma khach hang
go
...
begin
	...
	insert into NhanVien values(@makhachhang, @ho, @tenlot,@ten,@ngaysinh,@gioitinh,@sonha,@tenduong,@quan,@thanhpho,@dienthoai,@email,@matkhau)
end
--tang ma phim
go
create trigger trig_tangMaPhim on Phim
...
begin
	...
	declare @nhasanxuat nvarchar(50) = (select nhasanxuat from inserted)
	declare @thoiluong int = (select thoiluong from inserted)
	declare @trailer nvarchar(150) = (select trailer from inserted)
	
	insert into NhanVien values(@maphim, @matheloai, @daodien,@tenphim,@ngaykhoichieu,@ngayketthuc,@mota,@hinh,@nhasanxuat,@thoiluong,@trailer)
end
--tang ma phong chieu
go
create trigger trig_tangMaPhongChieu on PhongChieu
instead of insert
as
begin
	...
	insert into NhanVien values(@maphong, @tenphong, @soghebandau)
end
--tang ma suat chieu
go
create trigger trig_tangMaSuatChieu on SuatChieu
instead of insert
as
begin
	...
	insert into NhanVien values(@masuatchieu, @maphim, @maphong,@giochieu,@gioketthuc,@ngaychieu,@soghecontrong)
end

--tang ma theloai
go
create trigger trig_tangMaPhim on TheLoai
instead of insert
as
begin
	...
	insert into NhanVien values(@matheloai, @tentheloai)
end
